...
var_pick = read_excel("var_pick.xlsx")
```
...
The mapping is defined by matching the country codes in the 'cntry' column of the dataset to their corresponding regions. Subsequently, the 'factor()' function is applied to convert the 'region' column into a categorical variable, comprising predefined levels that signify the distinct regions. 

```{r}
cleaned_data <- ess_data
country_to_region <- c(
...
Next, specific variables are transformed into categorical or numeric formats based on the overview of the dataset's structure. 
...
```{r target_var}
ess_data = ess_data %>% select(-cntry)
