...
```{r r-import, warning=FALSE, message=FALSE}
# import package
## Data manipulation and visualization
library(tidyverse)
library(jsonlite)
...
edu = read_csv(file = "https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/education_data.csv")
...
```{r edu-final-processing, warning=FALSE}
...
full_edu <- edu %>%
  ...
  dplyr::select(employee_id, has_relevant_field, clean_degree, end_year) %>% 
  bind_rows(exp_phd) %>% 
  group_by(employee_id) %>%
...
data_train_level_1_e <- df_level_1_e[train_indices, ] %>% dplyr::select(-promote_general, -promote_level_2)
data_test_level_1_e <- df_level_1_e[-train_indices, ] %>% dplyr::select(-promote_general, -promote_level_2)
