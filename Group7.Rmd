...
biblio-style: apsr
title: "Career Promotion in Data Science, Statistics, and Artificial Intelligence: A Study of European University Graduates on LinkedIn"
author:
...
The number of connections and followers on LinkedIn is an indicator of networking skills and is relevant for various jobs, including recruiting, marketing, sales, or public relations (Zide et al., 2014). Having more connections makes it easier for individuals to be found by recruiters or hiring managers conducting keyword-based searches (Schneiderman, 2016). Recruiters can also analyze the connections of their existing employees to identify potential candidates for job vacancies (Caers & Castelyns, 2011). Therefore, building and nurturing professional connections can lead to job opportunities, mentorship, and other benefits. Based on the “connection” feature on LinkedIn, we will try to analyze and find certain networking effects.

Active engagement in networking activities, showcasing through the number of followers and connections on LinkedIn, may positively influence job opportunities, career mentorship, and professional growth prospects. We anticipate that graduates who establish strong networks may have an advantage in their career advancement.
...
We expect that graduates who have completed internships or work placements during their academic journey will have an advantage in the job market. Practical experience gained during internships may increase employability and lead to more favorable career trajectories compared to those without such experiences.

## Technical Skills and Career Advancement:
In the fields of statistics, data science, and artificial intelligence, technical skills are essential. According to Coursera, proficiency in programming, statistics and probability, machine learning, cloud computing, and related areas directly impacts career prospects. Identifying patterns in the skills held by graduates will provide insights into the essential skills for data-driven work.

We hypothesize a positive correlation between the level of technical proficiency and career growth. Graduates who possess advanced technical skills and expertise in relevant programming languages, tools, and methodologies are likely to experience faster career progression compared to those with more limited technical abilities.
...
1. What factors are significantly associated with the probability of receiving promotions in the data science, IT, software, and artificial intelligence fields? 
...
4. Are individuals who possess a bachelor's degree in fields not directly related to data science and IT (e.g., math, computer science, statistics, software engineering, etc.) more likely to pursue higher degrees in these fields compared to those who already had a bachelor's degree in related fields?
...
The essential data manipulation and transformation steps will be carried out in the R environment, involving cleaning, organizing, and structuring the data to ensure its suitability for analysis. To ensure the utmost ethical standards, we will anonymize the collected data by removing or pseudonymizing personally identifiable information to protect individuals' privacy in the dataset. 
...
To address the research questions, we will employ Random Forest and logistic regression. Moreover, to enhance interpretability, Random Forest will be combined with the SHAP (SHapley Additive exPlanations) technique, which will provide insights into the contribution of individual features to the models' predictions.
...
Regarding number of connections data, we observed that the number of connections on Linkedin is capped at 500, which means those with 500+ connections are still recorded as having 500 connections in our data set. Acknowledging this limitation, we chose to categorize the connection variable into a binary format, where '1' indicates the user possesses 500 or more connections, and '0' otherwise. Below is an outline of the code utilized for data processing.
...
From the results, it can be concluded that DeepFace achieved satisfactory overall performance with approximately 95% accuracy. However, it is worth noting that a significant number of misclassifications occurred specifically for women being identified as men. Notably, this misclassification trend was more prevalent among women of color, suggesting a potential bias in the dataset on which DeepFace was trained. 
...
- Topic 5 (Cloud Computing and Services): This topic appears to be related to cloud computing and services. It includes terms like AWS (Amazon Web Services), Azure, Google Cloud, and other web services, indicating a focus on cloud-based technologies and services.
...
## Are individuals who possess a bachelor’s degree in fields not directly related to data science and IT more likely to pursue higher degrees in these fields?
...
Out of the individuals who have completed their education in relevant fields, 80.4% of those holding either a bachelor's or master's degree; for those who have completed bachelor's or master's degrees in unrelated fields, the percentage is 86.2%. Moreover, the likelihood ratio test demonstrates a highly significant association between educational level (bachelor's or master's) and the relevance of the field of study (relevant or irrelevant). 

This finding suggests that many professionals in DS/AI field may find lucrative job opportunities and industry demand without the need to pursue further education at the master's level. It also implies that a bachelor's degree in DS/AI may be highly valued in the job market. Moreover, it demonstrates the keen interest of individuals from various educational backgrounds in pursuing advanced degrees in data science and AI-related subjects. However, it's important to note that the decision to pursue a master's degree can be influenced by various other factors, including personal career goals, industry requirements, and individual preferences.
...
We implement a Logistic Regression model to analyze the association between various features and the likelihood of employee promotions.

Firstly, several datasets are merged into a comprehensive dataset by the "employee_id". Records where time_work is greater than 0 are filtered out and some variables are transformed to appropriate data types. The data is split into training and testing sets, with 80% for training and 20% for testing. SMOTE (Synthetic Minority Over-sampling Technique) is applied to address class imbalance by oversampling the minority class. A logistic regression model is then fitted on the training data. 

The objective of the model is to forecast the 'promote_general' outcome, which indicates if an individual has been promoted to any higher job level. 
...
df_general <- exp_data %>%
  ...
  select(-skills, -skill_trans, -strongest_role, -skill4, -last_edu_year, -employee_id, -promote_level_1, -promote_level_2) %>%
...
smote_train <- ovun.sample(promote_general ~ ., data = data_smote, method = "both", N = length(data_smote), seed = 7)$data
...
```
The model results revealed several noteworthy inferences based on significant explanatory variables:

The variable "time_work" has a positive impact on the likelihood of an individual getting promoted to a higher job level, which suggests that individuals who have been working for a longer duration are more likely to receive promotions. 

Individuals with internship experience (intern) are less likely to get promoted to higher job levels compared to those without internship experience. This might indicate that internships might not be directly correlated with higher-level promotions in this context. It can also be seen in a way that those who are in senior positions would leave out intern in their profile, while mostly inexperienced start-up employees have nothing in their experience so they put the internships in. 

Moreover, it's interesting to see that number of previous jobs (num_job) shows a positive influence while number of previous company (num_company) shows negative influence, indicating that stability in employment is associated with higher chances of promotion.

Highest education level (highest_edu), German language proficiency (German), numbers of connections (connection_count), skill3 (Business Management and Strategy), and skill6 (Programming Language and Web Development Tools) are showing positive effects on the outcome, which help invalid our hypotheses. 

Last but not least, the variable "gender_predict" negatively impacts the likelihood of getting promoted -- woman are less likely to get promoted. This might indicate potential gender disparities in promotion opportunities. 
...
The model achieved an accuracy of approximately 73.23%. The positive predictive value (precision) is rather low (55.06%), meaning that only 55.06% of the individuals predicted to be promoted by the model actually got promoted. On the other hand, the negative predictive value (specificity) is around 86.40%, indicating that about 86.40% of the individuals predicted to not get promoted were correctly identified. The balanced accuracy, which takes into account both sensitivity and specificity, is approximately 73.60%.

However, there is room for improvement, especially in terms of precision. Further fine-tuning and investigation may be necessary to optimize the model's performance.


### Promotion level 1

In this section, we have developed Logistics Regression and Random Forest models to investigate the correlation between different factors and the probability of employees being promoted to level 1 positions, which include senior, lead, or principal roles. While examining the promotion patterns, we noticed that certain individuals bypass the level 1 position and are directly promoted to level 2 positions. In order to correctly classify these people who get promoted, we have excluded these cases from our models in this section. 
...
After data preparation, we proceeded to construct a Logistics Regression model using the training data. The model results revealed several noteworthy inferences based on significant explanatory variables, as outlined below:

- Work Experience and Job Mobility: Evidently, the length of an individual's work experience and the number of positions they have held exhibit a positive correlation with the likelihood of receiving a promotion. This implies that those who have worked for a longer duration and occupied diverse roles are more likely to attain higher positions. However, it was observed that excessive job-hopping, i.e., switching between numerous companies, hindered the chances of promotion.

- Education Level: The level of education also demonstrated a significant association with promotions. Individuals holding a PhD degree exhibited a higher likelihood of receiving promotions compared to others.
...
- Relevance of Skill Sets: The model revealed that possessing skill sets related only to Office Productivity and Microsoft Tools did not significantly improve one's chances of getting promoted in the Data Science field.
...
- Internship and Working Student Positions: Surprisingly, individuals with internship or working student positions were found to be less likely to receive promotions. This might be attributed to the fact that individuals in senior, lead, or principal positions may not prominently display their past internship experiences in their LinkedIn profiles.

Next, we constructed an XGBoost model to estimate the probability of receiving a promotion. The model-building process is detailed in the following code.
...
tunning_model_level1 = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/tunning_model_level1.csv")


if(!exists("tunning_model_level1")){
...
From the SHAP value plot, we observe that the number of positions held, working time, follower count, number of companies worked for, and variables related to skills are the most significant factors influencing the prediction. The impact of these explanatory variables on the prediction aligns with the pattern seen in the Logistic model. For instance, individuals with a higher number of followers on LinkedIn are more likely to have a greater chance of promotion.
...
## What are the distinguishing attributes between male and female professionials in the fields of interest?
...
model3_df <- 
  model3_df[, !(names(model3_df) %in% c("employee_id", "connection_count", "skills", "skill_trans", "skill4", "gender_predict", "last_edu_year"))]
...
model3_summary <- summary(step.model)
...
Among the most noteworthy findings are related to skills. All the skills (skill1 to skill8) are statistically significant in the model. A higher probability of having skills classified as skill2, skill5, skill6, and skill8 (related to programming languages, cloud computing, web development, and software engineering, AI/ML) is associated with a decreased likelihood of being classified as a woman. On the other hand, women more frequently list skills categorized as skill1 (data analysis and management) and skill7, which involves Microsoft tools and soft skills such as project management, teamwork, and communication.
...
Furthermore, there are certain limitations that are within our control. For instance, our findings may be specific to graduates from reputable European universities, making it less appropriate to extrapolate the results to other regions or academic institutions. Indeed, it is essential to recognize that while we gather data on European university alumni, our visibility into their career trajectories, particularly for non-EEA students, may not be comprehensive. This limitation means that we may not have a complete understanding of whether these individuals continue their careers within Europe or explore opportunities in their home countries. As a result, evaluating the relevance of proficiency in major European languages to job prospects in the European market might not yield entirely accurate results. Some countries may prioritize certain languages more than others in their job markets, leading to varying degrees of demand for specific language skills. Additionally, some students may choose to pursue opportunities in areas where proficiency in major European languages is not as essential for professional success, as they may be seeking opportunities in industries or positions where these languages are not prominently used.

In our study, we utilize gender classification based on profile photos as a means to address the absence of explicit gender information on LinkedIn profiles. However, it is vital to acknowledge that gender identity is a deeply personal and multifaceted aspect of an individual's identity. Relying solely on visual cues to classify gender may not always accurately reflect an individual's self-identified gender or encompass the diverse spectrum of gender identities that exist beyond traditional binary classifications. 

Additionally, our approach to identifying topics through "regular" LDA topic modelling may have limitations, and more complex modelling techniques could potentially yield additional insights. It's also important to note that topic modelling may not easily adapt to new or emerging skills in the industry, and as the tech field rapidly evolves, some skills may not be adequately represented in the current data.

Despite these limitations, our study still provides valuable insights into the career progression of graduates in the relevant fields. However, it is essential to acknowledge and consider these constraints when interpreting and applying the results. Additionally, it is important to acknowledge that other factors may influence the career progression of these graduates, such as location, race, educational background, and personal orientation. Future research can delve into these factors to enhance our understanding of successful career pathways in these fields.


# References
Skeels, M. M., & Grudin, J. (2009). When social networks cross boundaries. https://doi.org/10.1145/1531674.1531689
Lops, P., De Gemmis, M., Semeraro, G., Narducci, F., & Musto, C. (2011). Leveraging the linkedin social network data for extracting content-based user profiles. https://doi.org/10.1145/2043932.2043986
LinkedIn users, stats, data, trends, and more — DatarePortal – Global Digital insights. (n.d.). DataReportal – Global Digital Insights. https://datareportal.com/essential-linkedin-stats
Claybaugh, C. C., & Haseman, W. D. (2013). Understanding Professional Connections in Linkedin — A Question of Trust. Journal of Computer Information Systems, 54(1), 94–105. https://doi.org/10.1080/08874417.2013.11645675
Zide, J. S., Elman, B., & Shahani-Denning, C. (2014). LinkedIn and recruitment: how profiles differ across occupations. Employee Relations, 36(5), 583–604. https://doi.org/10.1108/er-07-2013-0086
Moss-Racusin, C. A., Dovidio, J. F., Brescoll, V. L., Graham, M., & Handelsman, J. (2012). Science faculty’s subtle gender biases favor male students. Proceedings of the National Academy of Sciences, 109(41), 16474–16479. https://doi.org/10.1073/pnas.1211286109
Watt, H. M. G., & Eccles, J. S. (2008). Gender and occupational outcomes: Longitudinal assessments of individual, social, and cultural influences. In American Psychological Association eBooks. https://doi.org/10.1037/11706-000
Kahn, S., & Ginther, D. K. (2017). Women and stem. https://doi.org/10.3386/w23525
Schneiderman, K. (2016). Using LinkedIn to connect. The Career Planning and Adult Development Journal, 32(3), 32. https://www.questia.com/library/journal/1P3-4171122291/using-linkedin-to-connect
Caers, R., & Castelyns, V. (2010). LinkedIn and Facebook in Belgium. Social Science Computer Review, 29(4), 437–448. https://doi.org/10.1177/0894439310386567
Rigsby, J. T., Addy, N., Herring, C., & Polledo, D. (2013). An Examination Of Internships And Job Opportunities. Journal of Applied Business Research, 29(4), 1131. https://doi.org/10.19030/jabr.v29i4.7921
Coursera. (2023). 7 skills every data scientist should have. Coursera. https://www.coursera.org/articles/data-scientist-skills
