...
keywords: "LinkedIn, data scientist, job skill, career progression, topic modelling, image processing, machine learning"
...
```{r python_setup, include=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r print-demo, echo=FALSE}
# Read the JSON file
json_data <- fromJSON("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/linkedin_scraper/profiles_demo.json")

# Print JSON 
pretty_json <- toJSON(json_data, pretty = TRUE)
cat(pretty_json)
```
...
```{python load-image-url, eval=FALSE}
...
```{python url-to-image, eval=FALSE}
image_url = "https://media.licdn.com/dms/image/C5603AQHHUuOSlRVA1w/profile-displayphoto-shrink_400_400/0/1579726624860?e=1698278400&v=beta&t=Ada1aikVFfwnXHb6Xd-d9FpJTPsNAGch2Jv5WdAAIMw"
img1 = url_to_image(image_url)
plt.imshow(img1[:,:,::-1])
plt.show()
```

```{python deepface-demo, eval=FALSE}
...
```{r print-gender-demo, out.width="50%", echo=FALSE}
url <- "https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/gender_classification.png"
...
edu = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/education_data.csv")
...
exp = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/experience_data.csv")
...
exp_phd <- subset(exp_phd, select=c(employee_id, clean_degree, end_year))
...
full_edu <- subset(full_edu, select=c(employee_id, clean_degree, end_year)) %>% 
  bind_rows(exp_phd) %>% 
  group_by(employee_id) %>%
...
# Load data
lang = read_csv('https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/language_data.csv')
...
lang_df_wide <- subset(lang_df_wide, select=c(employee_id, English, Dutch, German, Spanish, French, Chinese, Hindi)) %>% 
...
```{r exp-filter-field}
experience_data = read_csv('https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/experience_data.csv')
...
```{r skill-clean}
# Load data
skill <- read_csv('https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/skill_data_trans.csv')

# Remove NA lines
skill <-  skill %>% 
  filter (!is.na(skill$skill_trans))

knitr::kable(head(skill$skill_trans, 5))
...
# Load data
gender_data = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/final_processed_data/gender_processed.csv") %>% dplyr::select(employee_id, gender_predict)
edu_data = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/final_processed_data/edu_processed.csv")
lang_data = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/final_processed_data/lang_processed.csv")
follower_data = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/follower_data.csv")
connection_data = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/connection_data.csv")
skill_data = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/final_processed_data/skill_processed.csv")
exp_data = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/data_processing/final_processed_data/exp_processed.csv")
...
df = exp_data %>%
  ...
  dplyr::select(-skills, -skill_trans, -strongest_role, -skill4, -last_edu_year, -connection_count, -employee_id) %>%
...
data_train_level_1 <- df_level_1[train_indices_lv1, ] %>% dplyr::select(-promote_general, -promote_level_2)
data_test_level_1 <- df_level_1[-train_indices_lv1, ] %>% dplyr::select(-promote_general, -promote_level_2)
...
data_train_general <- df_general[train_indices_gen, ] %>% dplyr::select(-promote_level_1, -promote_level_2)
data_test_general <- df_general[-train_indices_gen, ] %>% dplyr::select(-promote_level_1, -promote_level_2)
...
data_train_level_1_e <- df_level_1_e[train_indices_lv1, ] %>% dplyr::select(-promote_general, -promote_level_2)
data_test_level_1_e <- df_level_1_e[-train_indices_lv1, ] %>% dplyr::select(-promote_general, -promote_level_2)
 


x_train_level_1_e <- data_train_level_1_e %>% dplyr::select(-promote_level_1)

x_test_level_1_e <- data_test_level_1_e %>% dplyr::select(-promote_level_1)
...
tunning_model_level1 = read_csv("https://raw.githubusercontent.com/tiendd712/Socialscience_bigdata_KUL/master/Assignment%203/tunning_model_level1.csv")
